...
void MainWindow::timerEvent(QTimerEvent *e) {
  ...
  // connect(ui->slider_linear_vel, SIGNAL(valueChanged(int)), ui->lineEdit,
  // SLOT(setLineEditValue(int)));
}
...
void MainWindow::on_pushButton_decstop_clicked() {
  double dectime[2] = {ui->textEdit_dectime->toPlainText().toDouble(),
                       ui->textEdit_dectime_2->toPlainText().toDouble()};
  short iret[2] = {0, 0};
  double actuvel[2];
  for (int i = 0; i < 2; i++) {
...
void MainWindow::on_pushButton_changepos_clicked() {
  WORD axisNo = 0;
  short iret = 0;
  double destpos[2] = {ui->textEdit_destpos->toPlainText().toDouble(),
                       ui->textEdit_destpos_2->toPlainText().toDouble()};
