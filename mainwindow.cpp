...
{
    QMessageBox::StandardButton reply;
    QString message[2] = {"左轮电机未使能,请先将电机使能"
                          "右轮电机未使能,请先将电机使能"};
...
bool MainWindow::enable_axis(int axisNo, int m_nConnectNo){
    short iret= 0;
...
bool MainWindow::disable_axis(int axisNo, int m_nConnectNo){
    short iret= 0;
...
void MainWindow::get_status(int axisNo, double *runvel, double *pulse){
    vel_limit(runvel);
    if(axisNo == 0)
    {
...
void MainWindow::move_axis(int axisNo){
    short statemachine=1;;
    ...
    iret = smc_set_profile_unit(0,axisNo,status[axisNo].startvel,status[axisNo].runvel,status[axisNo].acctime,status[axisNo].dectime,status[axisNo].stopvel);//设定单轴运动速度参数
...
{
    double runvel[2] = {ui->textEdit_runvel_0->toPlainText().toDouble(),
                        ui->textEdit_runvel_1->toPlainText().toDouble()};
    double pulse[2] = {ui->textEdit_pulse_0->toPlainText().toDouble(),
                       ui->textEdit_pulse_1->toPlainText().toDouble()};
...
void MainWindow::on_pushButton_decstop_0_clicked()
{
    ...
    double dectime[2] = {ui->textEdit_dectime_0->toPlainText().toDouble(),ui->textEdit_dectime_1->toPlainText().toDouble()};
    short iret[2]={0,0};
    double actuvel[2];
    for (int i=0;i<2;i++)
    {
...
void MainWindow::on_pushButton_changepos_clicked()
{
    WORD axisNo=0;
    short iret=0;
    double destpos[2] = {ui->textEdit_destpos_0->toPlainText().toDouble(),ui->textEdit_destpos_1->toPlainText().toDouble()};
...
void MainWindow::on_pushButton_start_wc_clicked()
{
    ...
    {
        double pulse[2]={0,0};
        get_status(0, runvel[1],pulse);
        get_status(1,runvel[1],pulse);

        move_axis(0);
        move_axis(1);

    }
    else if(ui->radioButton_fl->isChecked()) //开始定长运动
    {
        //定义电机在轮椅直线运动时的速度大小
